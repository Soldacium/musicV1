...
@Component({
  ...
  styleUrls: ['./player-playlist.component.css']
})
export class PlayerPlaylistComponent implements OnInit, AfterViewInit {
  ...
  render() {
      ...
      for ( let i = 0; i < this._albums.length; i++ ) {
          ...
          // before
          if ( i < this._index ) {
              this._albums[i].style[this._transformName] = 'translateX( -'+ ( this.OFFSET * ( this._index - i  ) ) +'% ) rotateY( '+ this.ROTATION +'deg )';
          ...
          if ( i === this._index ) {
           ...
           // after
          if ( i > this._index ) {
              this._albums[i].style[this._transformName] = 'translateX( '+ ( this.OFFSET * ( i - this._index  ) ) +'% ) rotateY( -'+ this.ROTATION +'deg )';
  ...
  keyDown( event ) {
      ...
      switch ( event.keyCode ) {
          case 37: this.flowRight(); break; // left
          case 39: this.flowLeft(); break; // right
      }
